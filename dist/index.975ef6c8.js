...
inputEl.addEventListener("input", (0, _lodashDebounceDefault.default)(onInputChange, DEBOUNCE_DELAY));
function onInputChange(e) {
    outputClear();
    if (e.target.value.trim() != "") (0, _fetchCountriesDefault.default)(e.target.value.trim()).then(renderHTML).catch((error)=>{
        (0, _notiflixNotifyAio.Notify).failure("Oops, there is no country with that name");
        console.log(error);
    });
}
...
function renderHTML(country) {
    ...
    else if (country.length >= 2 && country.length < 11) renderCountriesList(country);
    else if (country.length === 1) renderCountryCard(country);
}
function renderCountriesList(countriesArr) {
    const searchData = countriesArr.map(({ flags , name  })=>{
        return `<li>
...
function renderCountryCard(country) {
    const searchData = country.map(({ flags , name , capital , population , languages  })=>{
...
},{"./css/styles.css":"1CY4s","notiflix":"5WWYd","lodash.debounce":"3JP5n","./fetchCountries":"fTQqv","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3","notiflix/build/notiflix-notify-aio":"eXQLZ"}],"1CY4s":[function() {},{}],"5WWYd":[function(require,module,exports) {
var global = arguments[3];
...
},{}]},["ShInH","8lqZg"], "8lqZg", "parcelRequirec0c7")
